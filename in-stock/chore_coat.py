...
re_available = re.search(
    r"<button.*ProductForm__AddToCart.*Add to basket", response.text
)

if (re_sold_out is None) and (re_available is None):
    print(
        'ERROR: neither "sold out" or "available" regular expressions were matched',
        file=sys.stderr,
    )
    exit(1)
if (re_sold_out is not None) and (re_available is not None):
    print(
        'ERROR: both "sold out" and "available" regular expressions were matched',
        file=sys.stderr,
    )
    exit(1)
